...
- 节点编号必须与实际工作流中的节点 ID 匹配
- 节点状态修改是实时生效的，无需重新加载工作流
- 如果找不到指定编号的节点，会在控制台显示错误信息
- 确保 `web/check_node_bypass.js` 文件已正确加载（应该在 ComfyUI 启动时自动加载）
...
2. **检查 JavaScript 加载**：
   - 打开浏览器开发者工具（F12）
   - 查看控制台是否有 "AnyDoor 节点 Bypass 控制器已加载" 的日志
   - 如果没有，检查 `web/check_node_bypass.js` 文件是否存在

3. **检查浏览器控制台**：
   - 查看是否有错误信息
   - 查看是否有 "AnyDoor: 已设置节点 X 为模式 Y" 的成功日志
...
2. **前端脚本**（JavaScript）：
   - 监听控制节点的值变化
   - 实时查找并修改目标节点的 `mode` 属性
   - 在工作流执行前确保所有设置已应用

## 更新日志

### v2.0.0
- 重构为控制节点 bypass 状态的功能
- 添加实时控制支持
- 前端 JavaScript 自动处理节点状态修改
