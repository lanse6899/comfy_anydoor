...
# 导入 lazysheep 节点所需的模块
try:
    import comfy
    import torch
    import comfy.utils
    from server import PromptServer
    LAZYSHEEP_AVAILABLE = True
except ImportError:
    LAZYSHEEP_AVAILABLE = False

# AnyType 类用于支持任意类型的输入
class AnyType(str):
        ...
        return {
            "required": {
                ...
                "node_id1": ("STRING", {
                    "default": "",
                    "display_name": "节点ID1",
                    "tooltip": "第1组：对应的节点ID（可多个，支持逗号/空格/点分隔）"
                }),
                "node_id2": ("STRING", {
                    "default": "",
                    "display_name": "节点ID2",
                    "tooltip": "第2组：对应的节点ID（可多个，支持逗号/空格/点分隔）"
                }),
                "node_id3": ("STRING", {
                    "default": "",
                    "display_name": "节点ID3",
                    "tooltip": "第3组：对应的节点ID（可多个，支持逗号/空格/点分隔）"
                }),
                "node_id4": ("STRING", {
                    "default": "",
                    "display_name": "节点ID4",
                    "tooltip": "第4组：对应的节点ID（可多个，支持逗号/空格/点分隔）"
                }),
                "node_id5": ("STRING", {
                    "default": "",
                    "display_name": "节点ID5",
                    "tooltip": "第5组：对应的节点ID（可多个，支持逗号/空格/点分隔）"
                }),
            },
            "optional": {}
        }
        ...
        try:
            ...
            try:
                ...
                for nid in bypass_ids:
                    self._control_requests.append({'node_id': nid, 'mode': 2, 'mode_name': '忽略'})
            except:
                pass

            # 生成状态说明
            parts = [f"✓ 五键切换已应用", f"当前按钮: {idx}"]
            if bypass_ids:
                parts.append(f"忽略节点: {', '.join(map(str, bypass_ids))}")
            if restore_ids:
                parts.append(f"恢复节点: {', '.join(map(str, restore_ids))}")
            parts.append("注意: 实际设置由前端在执行/变更时应用")
            status = "\n".join(parts)

            # 返回 mode_value=2 表示本次主要动作为忽略
            return (status, 2 if bypass_ids else 0)
        except Exception as e:
            return (f"五键切换过程中出现错误: {str(e)}", 0)


# ============================================================================
# LazySheep 节点 - 惰性值机制节点
# ============================================================================

class LazyDiversioner:
    """
    惰性值机制的条件分支节点
    根据divert参数决定输出TRUE_IN或FALSE_IN，使用惰性值机制避免不必要的计算
    """
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                "divert": ("BOOLEAN", {"tooltip": "Invert the result of the condition post-evaluation"}),
            }, 
            "optional": {
    ...
    RETURN_NAMES = ("*",)
    OUTPUT_TOOLTIPS = ("Result of the comparison. Either a forwarded input depending on the divert value",)
    FUNCTION = "run_comparison"
    CATEGORY = "🔵BB anydoor"
    DESCRIPTION = "Lazy workflow diversion node with lazy evaluation"

    def check_lazy_status(s, divert, TRUE_IN=None, FALSE_IN=None, unique_id=0):
        """
        惰性值检查方法
        根据divert的值决定哪些输入需要实际计算
        """
        lazy = []
    ...
    def run_comparison(s, divert, TRUE_IN=None, FALSE_IN=None, unique_id=0):
        """
        执行条件分支
        根据divert的值返回对应的输入
        """
...
class LazyTextIndexer:
    """
    基于文本索引的惰性值机制节点
    根据文本索引匹配输入名称，只计算匹配的输入，使用惰性值机制避免不必要的计算
    """
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                "index": ("STRING", {"default": "", "tooltip": "文本索引，用于匹配输入名称"}),
            }, 
            "optional": {
                "input1_name": ("STRING", {"default": "", "tooltip": "输入1的名称"}),
                "input1": (any_type, {"forceInput": True, "lazy": True, "tooltip": "输入1的计算值"}),
                "input2_name": ("STRING", {"default": "", "tooltip": "输入2的名称"}),
                "input2": (any_type, {"forceInput": True, "lazy": True, "tooltip": "输入2的计算值"}),
                "input3_name": ("STRING", {"default": "", "tooltip": "输入3的名称"}),
                "input3": (any_type, {"forceInput": True, "lazy": True, "tooltip": "输入3的计算值"}),
                "input4_name": ("STRING", {"default": "", "tooltip": "输入4的名称"}),
                "input4": (any_type, {"forceInput": True, "lazy": True, "tooltip": "输入4的计算值"}),
                "input5_name": ("STRING", {"default": "", "tooltip": "输入5的名称"}),
                "input5": (any_type, {"forceInput": True, "lazy": True, "tooltip": "输入5的计算值"}),
            },
        }

    RETURN_TYPES = (any_type,)
    RETURN_NAMES = ("*",)
    OUTPUT_TOOLTIPS = ("根据索引匹配的输入值",)
    FUNCTION = "run_indexing"
    CATEGORY = "🔵BB anydoor"
    DESCRIPTION = "Lazy text indexer node with named inputs and lazy evaluation"

    def check_lazy_status(s, index, input1_name="", input1=None, input2_name="", input2=None, 
                         input3_name="", input3=None, input4_name="", input4=None, 
                         input5_name="", input5=None, unique_id=0):
        """
        惰性值检查方法
        根据索引匹配输入名称，决定哪些输入需要实际计算
        """
        lazy = []
        ...
        for input_key, name in inputs:
            if name and name.strip() == index.strip():
                lazy.append(input_key)
                break  # 找到匹配的就停止
        
        return lazy

    def run_indexing(s, index, input1_name="", input1=None, input2_name="", input2=None, 
                    input3_name="", input3=None, input4_name="", input4=None, 
                    input5_name="", input5=None, unique_id=0):
        """
        执行文本索引匹配
...
class LazySwitch:
    """
    基于数字切换的惰性值机制节点
    根据切换值(1-5)直接匹配对应的输入，只计算匹配的输入，使用惰性值机制避免不必要的计算
    """
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                "switch": ("INT", {"default": 1, "min": 1, "max": 5, "step": 1, "tooltip": "切换值，1-5对应input1-input5"}),
            }, 
            "optional": {
                "input1": (any_type, {"forceInput": True, "lazy": True, "tooltip": "输入1的计算值"}),
                "input2": (any_type, {"forceInput": True, "lazy": True, "tooltip": "输入2的计算值"}),
    ...
    RETURN_TYPES = (any_type,)
    RETURN_NAMES = ("*",)
    OUTPUT_TOOLTIPS = ("根据切换值匹配的输入值",)
    FUNCTION = "run_switch"
    CATEGORY = "🔵BB anydoor"
    DESCRIPTION = "Lazy switch node with numeric index (1-5) and lazy evaluation"

    def check_lazy_status(s, switch, input1=None, input2=None, input3=None, input4=None, input5=None, unique_id=0):
        """
        惰性值检查方法
        根据切换值决定哪些输入需要实际计算
        """
    ...
    def run_switch(s, switch, input1=None, input2=None, input3=None, input4=None, input5=None, unique_id=0):
        """
        执行切换匹配
        根据切换值返回对应的输入值
        """
